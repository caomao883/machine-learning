...
scores = np.hstack((np.ones((len(y_test),1))*1.0,np.ones((len(y_test),1))*0.0))

forest_fpr0, forest_tpr0, _ = metrics.roc_curve(label_binarize(y_test[names[-4]],classes=(0,1,2)).T[0:-1].T.ravel(), scores.ravel())
...
#AUC值
auc0 = metrics.auc(forest_fpr0, forest_tpr0)
