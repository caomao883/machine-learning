...
## 画图
final_step = [u'直径=1.7;n_lusters=None', u'直径=0.5;n_lusters=None', u'直径=1.7;n_lusters=100']
...
for ind, (birch_model, info) in enumerate(zip(birch_models, final_step)):
    ...
    print u"Birch算法，参数信息为：%s；模型构建消耗时间为:%.3f秒；聚类中心数目:%d" % (info, time_, len(np.unique(labels)))
